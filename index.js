...
setTimeout(() => {
    async function main() {
        ...
        const sockOptions = {
            ...
            browser: ['Mecha Orion-Md', "safari", "1.0.0"],
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            /* const Cy='2250500010852';
             const Ty='2250787700239';
             const Emoji='22 tu'*/
            /*  var superUser=[servBot,Cyber emoji,Tyler].map((s)=>s.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);
              var dev =[Cyber emoji,tyler].map((t)=>t.replace(/[^0-9]/g)+"@s.whatsapp.net").includes(auteurMessage);*/
            ...
            const superUserNumbers = [servBot, Cyber emoji, tyler, conf.NUMERO_OWNER].map((s) => s.replace(/[^0-9]/g) + "@s.whatsapp.net");
            const allAllowedNumbers = superUserNumbers.concat(sudo);
            const superUser = allAllowedNumbers.includes(auteurMessage);
            
            var dev = [Cyber Emoji,tyler].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
            function repondre(mes) { zk.sendMessage(origineMessage, { text: mes }, { quoted: ms }); }
            console.log("\t [][]...{Mecha Orion-md}...[][]");
...
function mybotpic() {
  ...
  try {
        ...
        if (texte.includes('https://') && verifGroupe &&  yes  ) {
         ...
         console.log("lien detecté")
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifMc-mdAdmin  ) { console.log('je fais rien'); return};
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} bouias du groupe.`;
                      var sticker = new Sticker(gifLink, {
                                    ...
                                    // var txt = `@${auteurMsgRepondu.split("@")[0]} a été bouias du groupe..\n`
                         ...
                         if ( warn >= warnlimit) { 
                          var kikmsg = `Lien detecté ; vous avez atteint le nombre maximal d'avertissement par consequant vous serrez bouias du groupe`;
                             ...
                             await zk.sendMessage(origineMessage, { delete: key });
                              ...
                              var  msg = `Lien detecté , vous avez un avertissement en plus au prochain tu sera bouias ;\n passez encore ${rest} avertissement(s) et vous serrez bouias du groupe`;
    ...
    let warnlimit = conf.WARN_COUNT
 if ( warn >= warnlimit) { 
  var kikmsg = `bot detecté ; vous avez atteint le nombre maximal d'avertissement par consequant vous serrez bouias du groupe`;
     ...
     await zk.sendMessage(origineMessage, { delete: key });
        ...
        var rest = warnlimit - warn ;
      var  msg = `bot detecté , vous avez un avertissement en plus au prochain tu sera bouias ;\n passez encore ${rest} avertissement(s) et vous serrez bouias du groupe`;
      ...
      await zk.sendMessage(origineMessage, { delete: key });
        ...
        console.log('.... ' + er);
            ...
            if (verifCom) {
                ...
                const cd = evt.cm.find((mecha orion) => mc.nomCom === (com));
                if (cd) {
                    try {
                        ...
                        console.log("😡😡 " + e);
                        mc.sendMessage(origineMessage, { text: "😡😡 " + e }, { quoted: ms });
                    }
                }
            }
...
${metadata.desc}`;
            ...
            MC.sendMessage(group.id, { image: { url: ppgroup }, caption: msg, mentions: membres });
...
/******** fin d'evenement groupe update *************************/
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                let cmsg = `╭《 𝗠𝗘𝗖𝗛𝗔 𝗢𝗥𝗜𝗢𝗡-𝗠𝗗 》───⊷
         ◎═•『*𖤍🂱✞*』•═◎
 ╭────────────
      《 𝗠𝗘𝗖𝗛𝗔 𝗢𝗥𝗜𝗢𝗡-𝗠𝗗 》
 ⁞ *Préfixe*:[${prefixe}] 
 ⁞ *Mode*:&{md}
 ⁞ *Nombre des   commandes*:${evt.cm.length}︎
⭓ ━━━ ◎💀◎ ━━━ ⭓
   https://chat.whatsapp.com/FD7qMppsHXkCSzbRqfRRcc
╰────────────
...
⭓ ━━━ ◎💀◎ ━━━ ⭓
╰────────────`;
                await MC.sendMessage(MC.user.id, { text: cmsg });
            }
        ...
        MC.ev.on("creds.update", saveCreds);
        //fin événement authentification 
        //
        /** ************* */
        //fonctions utiles
        MC.downloadAndSaveMediaMessage = async (message, filename = '', attachExtension = true) => {
